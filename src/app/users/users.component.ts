...
export class UsersComponent implements OnInit {
    ...
    if (RESPONSE !== undefined) {
      if (RESPONSE.permises !== undefined) {
        ...
        if (RESPONSE.ok) {
          ...
          this.displayedColumns = ['id_user', 'usuario', 'nombre_publico', 'pass_user', 'id_rol', 'token_sesion', 'lista_fav'];
  ...
  createFilter(): (usuario: User, filter: string) => boolean {
    const filterFunction = (usuario: User, filter: string): boolean => {
      ...
      return usuario.nombre_publico.toLowerCase().indexOf(searchTerms.nombre.toLowerCase()) !== -1
             && usuario.usuario.toLowerCase().indexOf(searchTerms.apellidos.toLowerCase()) !== -1
    };
