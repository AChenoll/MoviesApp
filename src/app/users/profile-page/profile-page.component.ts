...
export class ProfilePageComponent implements OnInit {
  ...
  async getIdsFavoritas() {
    const RESP = await this.favService.getFavorito(this.loca[0].id_usuario).toPromise();
    if (RESP !== undefined && RESP.permises !== undefined && RESP.ok) {
      this.arrayIdsMovies = RESP.data.map((item: { id_movie: any }) => item.id_movie);
  ...
  // Metodo para redireccionar al detalle de una pelicula favorita cuando pinchas sobre ella
  ...
  async quitarFavorita(id_fav: number | string) {
    const RESP = await this.favService.deleteFav(id_fav).toPromise();
